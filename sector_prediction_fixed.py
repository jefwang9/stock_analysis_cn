...
class SectorPredictionModel:
    ...
    def prepare_features(self, historical_data: Dict[str, pd.DataFrame], 
        ...
        for stock_code, stock_data in historical_data.items():
            ...
            if len(stock_data) > 1:
                ...
                target = (current_close - prev_close) / prev_close * 100
    ...
    def _extract_technical_features(self, stock_data: pd.DataFrame) -> Dict[str, float]:
        ...
        if len(stock_data) >= 5:
            features['volume_ma5'] = stock_data['Volume'].rolling(5).mean().iloc[-1]
    ...
    def _extract_market_features(self, stock_data: pd.DataFrame) -> Dict[str, float]:
        ...
        if len(stock_data) >= 10:
            ...
            features = {
                ...
                'price_momentum_5d': stock_data['Close'].pct_change(5).iloc[-1] if len(stock_data) >= 5 else 0,
                'trend_strength': abs(stock_data['Close'].rolling(10).mean().pct_change().iloc[-1]) if len(stock_data) >= 10 else 0,
                'gap_vs_yesterday': (current_price - stock_data['Open'].iloc[-1]) / stock_data['Open'].iloc[-1] if 'Open' in stock_data.columns else 0
            }
        else:
            features = {
                'price_position_10d': 0.5,
                'price_momentum_5d': 0,
                'trend_strength': 0,
                'gap_vs_yesterday': 0
            }
    ...
    def train_sector_model(self, sector: str, features_df: pd.DataFrame, 
        ...
        if features_df.empty or targets_series.empty:
            logger.warning(f"板块 {sector} 没有足够的数据进行训练")
        ...
        if len(X) < 10:  # 至少需要10个样本
            logger.warning(f"板块 {sector} 样本数量不足: {len(X)}")
            return {'status': 'error', 'message': '样本数量不足'}
        ...
        if XGBOOST_AVAILABLE:
            models_dict['xgb'] = xgb.XGBRegressor(n_estimators=100, random_state=42)
        
        if LIGHTGBM_AVAILABLE:
            models_dict['lightgbm'] = lgb.LGBMRegressor(n_estimators=100, random_state=42)
        ...
        if not trained_models:
            logger.error(f"板块 {sector} 所有模型训练失败")
...
def train_all_sector_models(all_features: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
    """训练所有板块的预测模型"""
    predictor = SectorPredictionModel()
    training_results = {}
